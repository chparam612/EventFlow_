...
export function render() {
  return `
  <div id="main-content" tabindex="-1" style="min-height:100vh;background:var(--bg-deep);display:flex;flex-direction:column;">
...
export async function init(navigate) {
  ...
  if (window._mapsReady) initCtrlMap();
  else window.addEventListener('mapsReady', initCtrlMap, { once: true });
  ...
  function updateMetrics(densities) {
    updateAnalytics(densities, currentEmergency);
  }
