...
await test('56. Firebase config has no placeholder values', () => {
  ...
  const firebaseCode = fs.readFileSync(path.join(projectRoot, 'src', 'firebase.js'), 'utf8');
...
await test('58. Accessibility: aria-live attributes defined in source', () => {
  const fs = createRequire(import.meta.url)('fs');
  const files = [
    path.join(projectRoot, 'src', 'panels', 'attendee', 'aiChat.js'),
    path.join(projectRoot, 'src', 'panels', 'control', 'dashboard.js'),
    path.join(projectRoot, 'src', 'panels', 'staff', 'dashboard.js')
  ];
...
await test('60. Accessibility: aria-live defined in at least one panel', () => {
  ...
  const filesToCheck = [
    ...
    path.join(projectRoot, 'src', 'panels', 'control', 'dashboard.js'),
    path.join(projectRoot, 'src', 'panels', 'staff', 'dashboard.js'),
    path.join(projectRoot, 'public', 'src', 'panels', 'attendee', 'aiChat.js'),
    path.join(projectRoot, 'public', 'src', 'panels', 'control', 'dashboard.js'),
    path.join(projectRoot, 'public', 'src', 'panels', 'staff', 'dashboard.js'),
  ];
  let found = false;
